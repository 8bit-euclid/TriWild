...
PYBIND11_MODULE(pytriwild, m) {
    ...
    m.def("triangulate", [](
        const Eigen::MatrixXd& vertices_in,
        const Eigen::MatrixXi& edges_in,
        py::dict feature_info_dict,
        double stop_quality = 10.0,
        int max_iterations = 80,
        int stage = 1,
        double epsilon = 1e-3,
        double feature_epsilon = 1e-3,
        double target_edge_length = -1.0,
        double edge_length_ratio = 0.02,
        double flat_feature_angle = 10.0,
        bool cut_outside = false,
        const Eigen::MatrixXd& hole_points = Eigen::MatrixXd(),
        bool mute_log = false
    ) {
        ...
        // Return as tuple
        return py::make_tuple(vertices_out, faces_out, nodes, face_nodes);
    },
    "Robust triangulation with curve constraints",
    py::arg("vertices_in"),
    py::arg("edges_in"), 
    py::arg("feature_info") = py::dict(),
    py::arg("stop_quality") = 10.0,
    py::arg("max_iterations") = 80,
    py::arg("stage") = 1,
    py::arg("epsilon") = 1e-3,
    py::arg("feature_epsilon") = 1e-3,
    py::arg("target_edge_length") = -1.0,
    py::arg("edge_length_ratio") = 0.02,
    py::arg("flat_feature_angle") = 10.0,
    py::arg("cut_outside") = false,
    py::arg("hole_points") = Eigen::MatrixXd(),
    py::arg("mute_log") = false
    );
